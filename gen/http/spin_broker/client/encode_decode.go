...
// BuildEnqueuedRequest instantiates a HTTP request object with method and path
// set to call the "spin-broker" service "enqueued" endpoint
func (c *Client) BuildEnqueuedRequest(ctx context.Context, v interface{}) (*http.Request, error) {
	var (
		id string
	)
	{
		p, ok := v.(*spinbroker.EnqueuedPayload)
		if !ok {
			return nil, goahttp.ErrInvalidType("spin-broker", "enqueued", "*spinbroker.EnqueuedPayload", v)
		}
		id = p.ID
	}
	u := &url.URL{Scheme: c.scheme, Host: c.host, Path: EnqueuedSpinBrokerPath(id)}
	req, err := http.NewRequest("POST", u.String(), nil)
	if err != nil {
		return nil, goahttp.ErrInvalidURL("spin-broker", "enqueued", u.String(), err)
	}
	if ctx != nil {
		req = req.WithContext(ctx)
	}
...
// DecodeEnqueuedResponse returns a decoder for responses returned by the
// spin-broker enqueued endpoint. restoreBody controls whether the response
// body should be restored after having been read.
func DecodeEnqueuedResponse(decoder func(*http.Response) goahttp.Decoder, restoreBody bool) func(*http.Response) (interface{}, error) {
	return func(resp *http.Response) (interface{}, error) {
		if restoreBody {
			b, err := ioutil.ReadAll(resp.Body)
			if err != nil {
		...
		case http.StatusOK:
			...
			err = decoder(resp).Decode(&body)
			if err != nil {
				return nil, goahttp.ErrDecodingError("spin-broker", "enqueued", err)
			}
			return body, nil
		default:
			body, _ := ioutil.ReadAll(resp.Body)
			return nil, goahttp.ErrInvalidResponse("spin-broker", "enqueued", resp.StatusCode, string(body))
		}
	}
}
